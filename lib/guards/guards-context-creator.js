...
class GuardsContextCreator extends context_creator_1.default {
    ...
    async tryActivate(guards, context) {
        if (!guards || shared_utils_1.isEmpty(guards)) {
            return true;
        }
        for (const guard of guards) {
            const args = this.routeParamsFactory.getArgsByMap(guard.argsMap, context, undefined);

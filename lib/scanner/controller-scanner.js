...
const fs = require("fs");
const project_path_enum_1 = require("../enums/project-path.enum");
class ControllerScanner {
    constructor() {
        this.metadataScanner = new medata_scanner_1.default();
    }
    scanFromDir() {
        const dir = path.resolve(); // 项目根目录
        const dirPath = path.join(dir, project_path_enum_1.ControlDir);
    ...
    scanForPaths(prototype) {
        return this.metadataScanner.scanFromPrototype(prototype, method => this.exploreMethodMetadata(prototype, method));
    }
}
